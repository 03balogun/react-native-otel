    ...
    for (const span of spans) {
      const duration =
        span.endTimeMs !== undefined
          ...
          : 'ongoing'
      const lines: string[] = [
        `[OTEL SPAN] ${span.name} (traceId=${span.traceId} spanId=${span.spanId})${span.parentSpanId ? ` parentSpanId=${span.parentSpanId}` : ''}`,
        `  duration: ${duration}  status: ${span.status}${span.statusMessage ? ` (${span.statusMessage})` : ''}`,
